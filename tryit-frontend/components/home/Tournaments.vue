<template>
  <div v-if="isTournamentsVisible">
    ...
    <h2>Tournaments</h2>
    <v-dialog v-model="isTournamentsVisible" max-width="600px" fullscreen>
      <v-card>
        ...
        </div><!-- Quitar cuando sepamos los horarios de los torneos-->
        ...
        <ul>
          <li v-for="tournament in tournaments" :key="tournament.id" style="list-style: none; margin: 20px;">
            ...
            <v-sheet class="toggleTournament" @click="logClick" color="secondary"
              ...
              <h3 class="title" style="color:white">{{ tournament.name }} <v-icon class="menu-icon"
                  style="color:white">mdi-menu-right</v-icon></h3>
              <p class="description" style="color:white; padding-bottom: 5px; white-space:pre-line; display:none"> {{
                tournament.description }}</p>
              <p class="date" style="color:white; padding-bottom: 5px"> Fecha inicio: {{ tournament.startDate }}
                <v-spacer></v-spacer> Fecha fin: {{ tournament.endDate }}
              </p>
              <p class="url" style="color:white; padding-bottom: 5px"> <a style="color:white"
                  :href="tournament.inscription_url">Apuntate Aquí!</a></p>
            </v-sheet>
          </li>
